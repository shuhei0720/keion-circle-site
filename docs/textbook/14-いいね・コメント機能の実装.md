...
`prisma/schema.prisma` を開いて、Likeモデルが**以下の通り定義されていること**を確認してください。
...
`src/app/posts/[id]/page.tsx` を開いて、以下の**2箇所を部分的に修正**してください。

> 💡 **説明**: このステップでは全体を上書きせず、import文の追加といいねボタンの追加のみを行います。
...
投稿詳細の表示部分で、メタ情報（日付、作成者など）の**下に**、いいねボタンを追加します。
...
`prisma/schema.prisma` を開いて、Commentモデルが**以下の通り定義されていること**を確認してください。
...
> 💡 **説明**: このステップでは全体を上書きせず、import文の追加とコメントセクションの追加のみを行います。

**修正1: import文を追加**

ファイルの先頭付近のimport文に、以下を**追加**:
...
投稿詳細の最後（いいねボタンの下）に、コメントセクションを**追加**します。
...
`src/app/posts/page.tsx` を開いて、カウント情報の部分が**以下のようになっていること**を確認してください。

> 💡 **確認**: Chapter 13で既に実装済みの場合はそのままでOKです。このセクションは確認のみで、コードの追加・変更は不要です。
