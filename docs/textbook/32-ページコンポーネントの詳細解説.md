...
## 32.1 ホームページ（src/app/page.tsx）

ホームページは、サイトのランディングページです。

### 全体構造

```typescript
'use client'  // Client Component（インタラクティブな要素が多いため）
...
export default function Home() {
  ...
  // ステート管理
  const [posts, setPosts] = useState<Post[]>([])        // 最新の投稿
  const [popularPosts, setPopularPosts] = useState<Post[]>([])  // 人気の投稿
...
【3. 配列操作】
  ...
  // スプレッド演算子 [...data] で元の配列を変更しない


【4. Tailwind CSSのグラデーション】
