...
このセクションでは、管理者が投稿を編集したり、イベントから活動報告を作成するためのページを解説します。

### 32.5.1 投稿編集ページ（src/app/posts/[id]/edit/page.tsx）

管理者が既存の投稿を編集するページです。RichTextEditorを使用して投稿内容を編集します。
...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 3. 投稿ID（Next.js 15+では params が Promise）
  const [postId, setPostId] = useState<string>('')
  
  // 4. RichTextEditor の参照
  ...
  // 6. params（Promise）を解決して postId に設定
  useEffect(() => {
  ...
  // 認証チェック中またはデータ取得中
  if (status === 'loading' || !postId || !formData.title) {
    return (
      <DashboardLayout>
        <div className="flex justify-center items-center min-h-screen">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <label className="cursor-pointer px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 flex items-center gap-2">
              ...
              {uploadingImages && <Loader2 className="animate-spin h-5 w-5" />}
            </div>
...
【既存データの取得】
  useEffect(() => {
    if (postId) {
      fetchPost()  // 投稿データを取得してフォームに設定
    }
  }, [postId])
  
  → 編集ページでは既存のデータを取得してフォームに表示
  → ユーザーは現在の内容を確認しながら編集できる
...
【UX の配慮】
  - ローディング中の表示（Loader2アイコン）
  - エラーメッセージの表示
  - 「戻る」ボタンでキャンセル
  - 更新完了後は投稿詳細ページへリダイレクト
```

---

### 32.5.2 イベント報告作成ページ（src/app/events/[id]/report/page.tsx）

イベントから活動報告を作成するページです。テンプレート機能を使用してイベント情報を活動報告に変換します。
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold">イベント報告を作成</h1>
          <p className="text-gray-600 mt-2">
            イベント情報をもとに活動報告を作成します
          </p>
        </div>

        {/* フォーム（投稿編集ページと同じ構造） */}
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* タイトル、内容、YouTube URL、画像アップロードの実装は省略 */}
          {/* 投稿編集ページと同じコードを使用 */}
...
【テンプレート機能】
  ...
  → URLパラメータからテンプレートを取得
  → イベント詳細ページで「テンプレートを生成」ボタンをクリックすると
     URLパラメータにテンプレートが含まれる
  → デコードしてフォームに設定


【テンプレートの構造】
  イベント情報をMarkdown形式に変換:
  ...
  → 管理者はこのテンプレートを編集して活動報告を作成


【デフォルトテンプレート】
  templateParam がない場合:
  
  1. イベント情報を取得してタイトルを設定
  2. デフォルトテンプレートを取得（/api/templates）
  3. フォームに設定
...
【活動報告の作成】
  ...
  → イベントとは別のリソース（投稿）として作成
  → イベントは予定、投稿は実施報告
  → 別々に管理することで柔軟性を確保


【活動スケジュールの報告作成ページ】
  src/app/activity-schedules/[id]/report/page.tsx
  
  → ほぼ同じ実装
  → eventId を activityScheduleId に変更
  → API エンドポイントを変更
```
