...
### Step 1: ナビゲーションコンポーネントを拡張

`src/components/Navbar.tsx` を開いて、以下のコードで**完全に上書き**してください。

> 💡 **説明**: Chapter 11で作成した基本的なNavbarを、レスポンシブデザイン（モバイル対応）とアイコン付きの完全版に拡張します。
...
> 💡 **説明**: Chapter 11で作成した基本的なレイアウトに、NavbarとFooterを追加した完全版に更新します。
...
`src/app/globals.css` を開いて、以下が含まれていることを確認してください。
...
`src/app/page.tsx` を開いて、以下のコードで**完全に上書き**してください。

> 💡 **説明**: ログイン状態に応じて表示が変わるホームページを作成します。
...
ログインしてから http://localhost:3000 を開いてください。
