...
Next.js の機能で、`src/app/api/` ディレクトリ内に API エンドポイントを作成できる。
...
Next.js 13 以降の新しいルーティングシステム。`src/app/` ディレクトリを使用。
...
**従来の Pages Router** (`src/pages/`) から移行中。
...
- useState, useEffect などの Hooks が使える
- イベントハンドラーが使える
...
- **PostgreSQL**: 高機能、リレーショナルデータベース、開発・本番環境の両方に適している
- **SQLite**: 軽量、ファイルベース（小規模プロジェクト向け）
- **MySQL**: 広く使われている、Web アプリに適している

**このプロジェクトでは**: PostgreSQL（Supabase でホスティング）を開発環境・本番環境の両方で使用

**なぜPostgreSQLを選んだのか：**
- 開発と本番で同じ環境を使うことでトラブルを防止
- Supabaseの無料プランで500MBまで利用可能
- リレーショナルデータベースとして高機能
- JSON型、配列型など豊富なデータ型に対応
...
サーバーで生成された HTML をクライアントで「生き返らせる」プロセス。
...
**Hydration Error**: サーバーとクライアントで HTML が一致しない場合に発生。
...
**このプロジェクトでは**: TypeScript を使用（JavaScript に型を追加）
...
**メリット**: HTML を離れずにスタイリング、一貫性のあるデザイン
...
[← 前の章：第 28 章 ライブラリとユーティリティの詳細解説](28-ライブラリとユーティリティの詳細解説.md) | [目次に戻る](00-目次.md) | [次の章へ：付録 B よく使うコマンド →](付録B-よく使うコマンド.md)
