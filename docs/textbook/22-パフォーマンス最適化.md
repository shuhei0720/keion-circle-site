...
`src/components/Avatar.tsx` を開いて、以下のコードで**完全に上書き**してください：

> 💡 **説明**: `priority` や `sizes` を追加して、画像読み込みを最適化します。
...
`src/app/posts/[id]/page.tsx` の投稿画像表示部分を、以下のように**部分的に修正**してください：

> 💡 **説明**: 投稿画像の表示部分だけを修正します。他のコードはそのままです。
...
`src/app/admin/posts/new/page.tsx` を開いて、以下のコードで**完全に上書き**してください：

> 💡 **説明**: `dynamic` を使って、Markdownエディターを必要なときだけ読み込むようにします。
...
`src/app/posts/page.tsx` を開いて、以下のコードで**完全に上書き**してください：

> 💡 **説明**: `cache` オプションを使って、データのキャッシング戦略を設定します。
...
`src/app/api/posts/route.ts` の POST メソッドを、以下のように**部分的に修正**してください：

> 💡 **説明**: 投稿作成後に `revalidatePath` でキャッシュを無効化し、最新のデータを表示します。
...
`src/app/layout.tsx` を開いて、Providers を使うように**部分的に修正**してください：

> 💡 **説明**: 既存の layout.tsx に Providers を追加します。他のコードはそのままです。
