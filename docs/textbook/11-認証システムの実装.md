...
`.env.local` ファイルを開いて、以下の認証関連の設定が**すべて含まれていること**を確認してください。

> 💡 **確認**: Chapter 8で設定済みの場合はそのままでOKです。不足している項目があれば追記してください。
...
`prisma/schema.prisma` を開いて、以下の認証関連のモデルが**すべて定義されていること**を確認してください。
...
### Step 3: NextAuth.js の設定ファイルを作成

`src/lib/auth.ts` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/api/auth/[...nextauth]/route.ts` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/layout.tsx` を開いて、以下のコードで**完全に上書き**してください。

> 💡 **説明**: SessionProviderを追加して、アプリ全体でセッション情報を使えるようにします。
...
`src/lib/auth.ts` を開いて、ファイルの**最後に以下を追加**してください。

> 💡 **説明**: 既存のコードはそのままで、管理者チェック関数を追加します。
