...
`package.json` を開いて、以下のスクリプトを追加してください：
...
`src/lib/__tests__` フォルダに `test-helpers.ts` を開いて、以下を入力：
│ =file:test.db│
└─────────────┘
       ↓
  ✓ 本番DBを汚さない
...
### Step 4: `src/app/api/posts/__tests__/route.integration.test.ts` を開いて、以下を入力
