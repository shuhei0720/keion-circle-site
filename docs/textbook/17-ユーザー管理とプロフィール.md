# 第17章：ユーザー管理とプロフィール【ハンズオン】

> **この章では、メンバー管理とプロフィール編集機能を実装します**

## 🛠️ **ハンズオン作業**

**⚠️ 重要**: この章では実際に手を動かしてユーザー管理機能を実装します。各セクションの指示に従って、メンバー一覧・プロフィール編集・役割管理機能を作っていきましょう。

## 📚 この章で学ぶこと

- ✅ ユーザー一覧ページの実装
- ✅ 管理者による役割変更機能
- ✅ プロフィール編集機能
- ✅ アバター画像のアップロード（Supabase Storage）
- ✅ ユーザー詳細ページの実装
- ✅ 活動履歴の表示

## 💡 この章で実装する機能

```
┌──────────────────────────────────────────────────┐
│       ユーザー管理機能の全体像                    │
└──────────────────────────────────────────────────┘

【メンバー一覧】
全メンバーの表示、役割の確認
   ↓
【管理者機能】
役割変更（member ⇄ admin）
   ↓
【プロフィール編集】
名前、自己紹介、担当楽器、アバター画像
   ↓
【ユーザー詳細】
プロフィール情報、活動履歴


┌────────────────────────────────────┐
│   メンバー一覧                      │
│                                    │
│   [🎸 山田太郎]  管理者            │
│   [🎤 佐藤花子]  メンバー          │
│   [🥁 鈴木一郎]  メンバー          │
│                                    │
│   → クリックで詳細ページへ        │
└────────────────────────────────────┘

┌────────────────────────────────────┐
│   プロフィール編集                  │
│                                    │
│   [アバター画像]                   │
│   📷 画像を変更                    │
│                                    │
│   名前: [山田太郎]                 │
│   自己紹介: [ギター担当です]       │
│   担当楽器: [ギター]               │
│                                    │
│   [保存]                           │
└────────────────────────────────────┘
```

---

## 17.1 ユーザー一覧ページの実装

### データ取得の設計

ユーザー一覧では、以下の情報を表示します：

```
┌──────────────────────────────────────┐
│     ユーザー一覧で表示する情報        │
├──────────────────────────────────────┤
│ ✓ アバター画像                       │
│ ✓ ユーザー名                         │
│ ✓ 役割（admin / member）             │
│ ✓ 自己紹介                           │
│ ✓ 担当楽器                           │
│ ✓ 登録日                             │
└──────────────────────────────────────┘
```

### Step 1: ユーザー一覧ページを作成

`src/app/users/page.tsx` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { prisma } from '@/lib/db';
import { auth } from '@/lib/auth';
import Link from 'next/link';
import Image from 'next/image';
import { Users, Shield, User } from 'lucide-react';

export default async function UsersPage() {
  const session = await auth();

  // 全ユーザーを取得
  const users = await prisma.user.findMany({
    select: {
      id: true,
      name: true,
      email: true,
      image: true,
      role: true,
      bio: true,
      instrument: true,
      createdAt: true,
      // 統計情報
      _count: {
        select: {
          posts: true,
          participations: true,
          comments: true,
        },
      },
    },
    orderBy: [
      { role: 'desc' }, // adminを先に
      { createdAt: 'asc' }, // 古い順
    ],
  });

  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* ヘッダー */}
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            <Users size={32} className="text-blue-600" />
            <h1 className="text-3xl font-bold text-gray-900">メンバー一覧</h1>
          </div>
          <p className="text-gray-600">
            BOLD軽音のメンバーを確認できます
          </p>
        </div>

        {/* ユーザー数の表示 */}
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-blue-900 font-medium">
            総メンバー数: {users.length}名
            {users.filter((u) => u.role === 'admin').length > 0 && (
              <span className="ml-4 text-blue-700">
                （管理者: {users.filter((u) => u.role === 'admin').length}名）
              </span>
            )}
          </p>
        </div>

        {/* ユーザーグリッド */}
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {users.map((user) => (
            <UserCard key={user.id} user={user} isAdmin={session?.user?.role === 'admin'} />
          ))}
        </div>
      </div>
    </div>
  );
}

// ユーザーカードコンポーネント
function UserCard({ user, isAdmin }: { user: any; isAdmin: boolean }) {
  return (
    <Link
      href={`/users/${user.id}`}
      className="block bg-white rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden"
    >
      {/* ヘッダー（役割に応じて色分け） */}
      <div
        className={`h-20 ${
          user.role === 'admin'
            ? 'bg-gradient-to-r from-purple-600 to-indigo-600'
            : 'bg-gradient-to-r from-blue-500 to-cyan-500'
        }`}
      />

      {/* コンテンツ */}
      <div className="p-6 -mt-12">
        {/* アバター */}
        <div className="flex items-start justify-between mb-4">
          <div className="relative">
            {user.image ? (
              <Image
                src={user.image}
                alt={user.name || 'User'}
                width={80}
                height={80}
                className="rounded-full border-4 border-white shadow-lg"
              />
            ) : (
              <div className="w-20 h-20 rounded-full bg-gray-300 border-4 border-white shadow-lg flex items-center justify-center">
                <User size={40} className="text-gray-600" />
              </div>
            )}

            {/* 役割バッジ */}
            {user.role === 'admin' && (
              <div className="absolute -bottom-1 -right-1 bg-yellow-400 rounded-full p-1.5 border-2 border-white">
                <Shield size={16} className="text-yellow-900" />
              </div>
            )}
          </div>

          {/* 管理者マーク（テキスト） */}
          {user.role === 'admin' && (
            <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs font-bold rounded">
              管理者
            </span>
          )}
        </div>

        {/* ユーザー名 */}
        <h3 className="text-xl font-bold text-gray-900 mb-2">
          {user.name || 'Unknown User'}
        </h3>

        {/* 自己紹介 */}
        {user.bio && (
          <p className="text-gray-600 text-sm mb-3 line-clamp-2">{user.bio}</p>
        )}

        {/* 担当楽器 */}
        {user.instrument && (
          <div className="mb-3">
            <span className="inline-block px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">
              {user.instrument}
            </span>
          </div>
        )}

        {/* 統計情報 */}
        <div className="flex items-center gap-4 text-sm text-gray-600 pt-3 border-t border-gray-200">
          <div>
            <span className="font-semibold">{user._count.posts}</span>
            <span className="ml-1">投稿</span>
          </div>
          <div>
            <span className="font-semibold">{user._count.participations}</span>
            <span className="ml-1">参加</span>
          </div>
          <div>
            <span className="font-semibold">{user._count.comments}</span>
            <span className="ml-1">コメント</span>
          </div>
        </div>
      </div>
    </Link>
  );
}
```

**ユーザー一覧の特徴:**

1. ✅ **役割による色分け**: 管理者は紫、メンバーは青
2. ✅ **アバター画像**: プロフィール画像を表示
3. ✅ **統計情報**: 投稿数、参加数、コメント数
4. ✅ **カード形式**: グリッドレイアウト
5. ✅ **クリックで詳細**: ユーザー詳細ページへ

### Step 2: 動作確認

ブラウザで http://localhost:3000/users を開いて、以下のコードを**すべて**入力してください：

以下のコードで**上書き**してください：

→ ✅ メンバー一覧が表示されます！

---

## 17.2 管理者機能の実装

### 役割変更の仕組み

```
┌──────────────────────────────────────────────┐
│         役割変更機能                          │
└──────────────────────────────────────────────┘

【権限】
管理者のみが他のユーザーの役割を変更可能

【変更パターン】
member → admin  昇格
admin  → member 降格

【制約】
自分自身の役割は変更不可
（最後の管理者を失わないため）
```

### Step 1: 役割変更APIを実装

`src/app/api/users/[id]/role/route.ts` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { NextResponse } from 'next/server';
import { auth } from '@/lib/auth';
import { prisma } from '@/lib/db';

/**
 * ユーザー役割の変更（PATCH）
 */
export async function PATCH(
  request: Request,
  { params }: { params: { id: string } }
) {
  try {
    // 認証チェック
    const session = await auth();
    if (!session?.user) {
      return NextResponse.json(
        { error: 'ログインが必要です' },
        { status: 401 }
      );
    }

    // 管理者権限チェック
    if (session.user.role !== 'admin') {
      return NextResponse.json(
        { error: '管理者権限が必要です' },
        { status: 403 }
      );
    }

    const userId = params.id;
    const body = await request.json();
    const { role } = body; // 'admin' or 'member'

    // バリデーション
    if (role !== 'admin' && role !== 'member') {
      return NextResponse.json(
        { error: '無効な役割です' },
        { status: 400 }
      );
    }

    // 自分自身の役割は変更不可
    if (userId === session.user.id) {
      return NextResponse.json(
        { error: '自分自身の役割は変更できません' },
        { status: 400 }
      );
    }

    // 対象ユーザーが存在するか確認
    const targetUser = await prisma.user.findUnique({
      where: { id: userId },
    });

    if (!targetUser) {
      return NextResponse.json(
        { error: 'ユーザーが見つかりません' },
        { status: 404 }
      );
    }

    // 役割を更新
    const updatedUser = await prisma.user.update({
      where: { id: userId },
      data: { role },
      select: {
        id: true,
        name: true,
        email: true,
        role: true,
      },
    });

    return NextResponse.json({
      user: updatedUser,
      message: `役割を${role === 'admin' ? '管理者' : 'メンバー'}に変更しました`,
    });
  } catch (error) {
    console.error('役割変更エラー:', error);
    return NextResponse.json(
      { error: '役割の変更に失敗しました' },
      { status: 500 }
    );
  }
}
```

### Step 2: ユーザー詳細ページを作成

`src/app/users/[id]/page.tsx` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { prisma } from '@/lib/db';
import { auth } from '@/lib/auth';
import { notFound } from 'next/navigation';
import Link from 'next/link';
import Image from 'next/image';
import {
  User,
  Mail,
  Calendar,
  Music,
  FileText,
  Users as UsersIcon,
  MessageCircle,
  Shield,
  Edit,
} from 'lucide-react';
import { RoleChangeButton } from '@/components/RoleChangeButton';

export default async function UserDetailPage({
  params,
}: {
  params: { id: string };
}) {
  const session = await auth();

  // ユーザー情報を取得
  const user = await prisma.user.findUnique({
    where: { id: params.id },
    include: {
      // 投稿
      posts: {
        take: 5,
        orderBy: { createdAt: 'desc' },
        include: {
          _count: {
            select: {
              likes: true,
              comments: true,
            },
          },
        },
      },
      // イベント参加
      participations: {
        take: 5,
        orderBy: { createdAt: 'desc' },
        include: {
          event: true,
        },
      },
      // コメント
      comments: {
        take: 5,
        orderBy: { createdAt: 'desc' },
        include: {
          post: {
            select: {
              id: true,
              title: true,
            },
          },
        },
      },
      // 統計
      _count: {
        select: {
          posts: true,
          participations: true,
          comments: true,
          likes: true,
        },
      },
    },
  });

  if (!user) {
    notFound();
  }

  // 編集権限（本人または管理者）
  const canEdit =
    session?.user?.id === user.id || session?.user?.role === 'admin';

  // 役割変更権限（管理者のみ、自分以外）
  const canChangeRole =
    session?.user?.role === 'admin' && session?.user?.id !== user.id;

  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* 戻るリンク */}
        <Link
          href="/users"
          className="inline-flex items-center text-blue-600 hover:text-blue-700 mb-6"
        >
          ← メンバー一覧に戻る
        </Link>

        {/* プロフィールカード */}
        <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-8">
          {/* ヘッダー（役割に応じて色分け） */}
          <div
            className={`h-32 ${
              user.role === 'admin'
                ? 'bg-gradient-to-r from-purple-600 to-indigo-600'
                : 'bg-gradient-to-r from-blue-500 to-cyan-500'
            }`}
          />

          {/* プロフィール情報 */}
          <div className="px-8 pb-8">
            <div className="flex flex-col md:flex-row items-start md:items-end gap-6 -mt-16 mb-6">
              {/* アバター */}
              <div className="relative">
                {user.image ? (
                  <Image
                    src={user.image}
                    alt={user.name || 'User'}
                    width={120}
                    height={120}
                    className="rounded-full border-4 border-white shadow-lg"
                  />
                ) : (
                  <div className="w-30 h-30 rounded-full bg-gray-300 border-4 border-white shadow-lg flex items-center justify-center">
                    <User size={60} className="text-gray-600" />
                  </div>
                )}

                {/* 役割バッジ */}
                {user.role === 'admin' && (
                  <div className="absolute -bottom-2 -right-2 bg-yellow-400 rounded-full p-2 border-2 border-white">
                    <Shield size={20} className="text-yellow-900" />
                  </div>
                )}
              </div>

              {/* 名前と役割 */}
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="text-3xl font-bold text-gray-900">
                    {user.name || 'Unknown User'}
                  </h1>
                  {user.role === 'admin' && (
                    <span className="px-3 py-1 bg-yellow-100 text-yellow-800 text-sm font-bold rounded">
                      管理者
                    </span>
                  )}
                </div>

                {/* 担当楽器 */}
                {user.instrument && (
                  <div className="flex items-center gap-2 text-gray-600 mb-2">
                    <Music size={18} />
                    <span className="font-medium">{user.instrument}</span>
                  </div>
                )}

                {/* メールアドレス */}
                <div className="flex items-center gap-2 text-gray-600">
                  <Mail size={18} />
                  <span>{user.email}</span>
                </div>
              </div>

              {/* ボタン */}
              <div className="flex gap-2">
                {/* 編集ボタン */}
                {canEdit && (
                  <Link
                    href="/profile"
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2"
                  >
                    <Edit size={18} />
                    プロフィール編集
                  </Link>
                )}

                {/* 役割変更ボタン */}
                {canChangeRole && (
                  <RoleChangeButton userId={user.id} currentRole={user.role} />
                )}
              </div>
            </div>

            {/* 自己紹介 */}
            {user.bio && (
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-semibold text-gray-900 mb-2">自己紹介</h3>
                <p className="text-gray-700 whitespace-pre-wrap">{user.bio}</p>
              </div>
            )}

            {/* 統計情報 */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="p-4 bg-blue-50 rounded-lg text-center">
                <FileText className="mx-auto mb-2 text-blue-600" size={24} />
                <div className="text-2xl font-bold text-blue-900">
                  {user._count.posts}
                </div>
                <div className="text-sm text-blue-700">投稿</div>
              </div>

              <div className="p-4 bg-green-50 rounded-lg text-center">
                <UsersIcon className="mx-auto mb-2 text-green-600" size={24} />
                <div className="text-2xl font-bold text-green-900">
                  {user._count.participations}
                </div>
                <div className="text-sm text-green-700">イベント参加</div>
              </div>

              <div className="p-4 bg-purple-50 rounded-lg text-center">
                <MessageCircle
                  className="mx-auto mb-2 text-purple-600"
                  size={24}
                />
                <div className="text-2xl font-bold text-purple-900">
                  {user._count.comments}
                </div>
                <div className="text-sm text-purple-700">コメント</div>
              </div>

              <div className="p-4 bg-red-50 rounded-lg text-center">
                <Calendar className="mx-auto mb-2 text-red-600" size={24} />
                <div className="text-sm text-red-700">
                  {new Date(user.createdAt).toLocaleDateString('ja-JP')}
                </div>
                <div className="text-sm text-red-700">登録日</div>
              </div>
            </div>
          </div>
        </div>

        {/* 活動履歴 */}
        <div className="grid md:grid-cols-2 gap-8">
          {/* 最近の投稿 */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              <FileText size={24} className="text-blue-600" />
              最近の投稿
            </h2>

            {user.posts.length > 0 ? (
              <div className="space-y-3">
                {user.posts.map((post) => (
                  <Link
                    key={post.id}
                    href={`/posts/${post.id}`}
                    className="block p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
                  >
                    <h3 className="font-semibold text-gray-900 mb-1">
                      {post.title}
                    </h3>
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <span>👍 {post._count.likes}</span>
                      <span>💬 {post._count.comments}</span>
                      <span>
                        {new Date(post.createdAt).toLocaleDateString('ja-JP')}
                      </span>
                    </div>
                  </Link>
                ))}
              </div>
            ) : (
              <p className="text-gray-500 text-center py-8">投稿がありません</p>
            )}
          </div>

          {/* 最近のイベント参加 */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              <UsersIcon size={24} className="text-green-600" />
              最近のイベント参加
            </h2>

            {user.participations.length > 0 ? (
              <div className="space-y-3">
                {user.participations.map((participation) => (
                  <Link
                    key={participation.id}
                    href={`/events/${participation.event.id}`}
                    className="block p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
                  >
                    <h3 className="font-semibold text-gray-900 mb-1">
                      {participation.event.title}
                    </h3>
                    <div className="text-sm text-gray-600">
                      {new Date(participation.event.date).toLocaleDateString(
                        'ja-JP',
                        {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                          weekday: 'short',
                        }
                      )}
                    </div>
                  </Link>
                ))}
              </div>
            ) : (
              <p className="text-gray-500 text-center py-8">
                参加イベントがありません
              </p>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}
```

### Step 3: 役割変更ボタンコンポーネントを作成

`src/components/RoleChangeButton.tsx` を作成して、以下のコードを**すべて**入力してください：

```typescript
'use client';

import { useState, useTransition } from 'react';
import { useRouter } from 'next/navigation';
import { Shield, ShieldOff } from 'lucide-react';

interface RoleChangeButtonProps {
  userId: string;
  currentRole: string;
}

export function RoleChangeButton({ userId, currentRole }: RoleChangeButtonProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [isChanging, setIsChanging] = useState(false);

  const handleRoleChange = async () => {
    const newRole = currentRole === 'admin' ? 'member' : 'admin';
    const confirmMessage =
      newRole === 'admin'
        ? 'このユーザーを管理者に昇格しますか？'
        : 'このユーザーを一般メンバーに降格しますか？';

    if (!confirm(confirmMessage)) {
      return;
    }

    setIsChanging(true);

    try {
      const response = await fetch(`/api/users/${userId}/role`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: newRole }),
      });

      const data = await response.json();

      if (!response.ok) {
        alert(data.error || '役割の変更に失敗しました');
        setIsChanging(false);
        return;
      }

      alert(data.message);

      // ページを再検証
      startTransition(() => {
        router.refresh();
      });
    } catch (error) {
      alert('役割の変更に失敗しました');
      setIsChanging(false);
    }
  };

  return (
    <button
      onClick={handleRoleChange}
      disabled={isChanging || isPending}
      className={`
        px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2
        ${
          currentRole === 'admin'
            ? 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            : 'bg-purple-600 text-white hover:bg-purple-700'
        }
        disabled:opacity-50 disabled:cursor-not-allowed
      `}
    >
      {currentRole === 'admin' ? (
        <>
          <ShieldOff size={18} />
          降格
        </>
      ) : (
        <>
          <Shield size={18} />
          昇格
        </>
      )}
    </button>
  );
}
```

### Step 4: 動作確認

**4-1.** 管理者ユーザーでログイン

**4-2.** http://localhost:3000/users を開いてメンバーをクリック

**4-3.** ユーザー詳細ページで「昇格」または「降格」ボタンをクリック

→ ✅ 役割が変更されます！

---

## 17.3 プロフィール編集ページの実装

### プロフィール編集の仕組み

```
┌──────────────────────────────────────────────┐
│         プロフィール編集機能                  │
└──────────────────────────────────────────────┘

【編集可能な項目】
✓ アバター画像（Supabase Storageにアップロード）
✓ 名前
✓ 自己紹介
✓ 担当楽器

【アップロードの流れ】
1️⃣ 画像を選択
2️⃣ プレビュー表示
3️⃣ Supabase Storageにアップロード
4️⃣ URLを取得
5️⃣ データベースに保存
```

### Step 1: プロフィール更新APIを実装

`src/app/api/profile/route.ts` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { NextResponse } from 'next/server';
import { auth } from '@/lib/auth';
import { prisma } from '@/lib/db';

/**
 * プロフィール更新（PATCH）
 */
export async function PATCH(request: Request) {
  try {
    // 認証チェック
    const session = await auth();
    if (!session?.user) {
      return NextResponse.json(
        { error: 'ログインが必要です' },
        { status: 401 }
      );
    }

    const body = await request.json();
    const { name, bio, instrument, image } = body;

    // バリデーション
    if (name && name.trim().length === 0) {
      return NextResponse.json(
        { error: '名前を入力してください' },
        { status: 400 }
      );
    }

    // プロフィールを更新
    const updatedUser = await prisma.user.update({
      where: { id: session.user.id },
      data: {
        ...(name && { name: name.trim() }),
        ...(bio !== undefined && { bio: bio.trim() || null }),
        ...(instrument !== undefined && { instrument: instrument.trim() || null }),
        ...(image !== undefined && { image: image.trim() || null }),
      },
      select: {
        id: true,
        name: true,
        email: true,
        image: true,
        bio: true,
        instrument: true,
      },
    });

    return NextResponse.json({
      user: updatedUser,
      message: 'プロフィールを更新しました',
    });
  } catch (error) {
    console.error('プロフィール更新エラー:', error);
    return NextResponse.json(
      { error: 'プロフィールの更新に失敗しました' },
      { status: 500 }
    );
  }
}
```

### Step 2: Supabaseクライアントを作成

`src/lib/supabase.ts` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { createClient } from '@supabase/supabase-js';

// Supabaseクライアントの作成
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;

export const supabase = createClient(supabaseUrl, supabaseAnonKey);

/**
 * アバター画像をアップロード
 */
export async function uploadAvatar(file: File, userId: string): Promise<string> {
  // ファイル名を生成（重複を避けるためタイムスタンプを追加）
  const fileExt = file.name.split('.').pop();
  const fileName = `${userId}-${Date.now()}.${fileExt}`;
  const filePath = `avatars/${fileName}`;

  // ファイルをアップロード
  const { data, error } = await supabase.storage
    .from('avatars')
    .upload(filePath, file, {
      cacheControl: '3600',
      upsert: false,
    });

  if (error) {
    console.error('アップロードエラー:', error);
    throw new Error('画像のアップロードに失敗しました');
  }

  // 公開URLを取得
  const { data: publicUrlData } = supabase.storage
    .from('avatars')
    .getPublicUrl(filePath);

  return publicUrlData.publicUrl;
}

/**
 * 古いアバター画像を削除
 */
export async function deleteAvatar(imageUrl: string): Promise<void> {
  try {
    // URLからファイルパスを抽出
    const url = new URL(imageUrl);
    const filePath = url.pathname.split('/storage/v1/object/public/avatars/')[1];

    if (!filePath) {
      return;
    }

    // 画像を削除
    const { error } = await supabase.storage.from('avatars').remove([`avatars/${filePath}`]);

    if (error) {
      console.error('削除エラー:', error);
    }
  } catch (error) {
    console.error('削除処理エラー:', error);
  }
}
```

### Step 3: プロフィール編集ページを作成

`src/app/profile/page.tsx` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { auth } from '@/lib/auth';
import { redirect } from 'next/navigation';
import { prisma } from '@/lib/db';
import { ProfileForm } from '@/components/ProfileForm';

export default async function ProfilePage() {
  const session = await auth();

  // 認証チェック
  if (!session) {
    redirect('/auth/signin');
  }

  // ユーザー情報を取得
  const user = await prisma.user.findUnique({
    where: { id: session.user.id },
    select: {
      id: true,
      name: true,
      email: true,
      image: true,
      bio: true,
      instrument: true,
    },
  });

  if (!user) {
    redirect('/auth/signin');
  }

  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">
          プロフィール編集
        </h1>

        <ProfileForm user={user} />
      </div>
    </div>
  );
}
```

### Step 4: プロフィールフォームコンポーネントを作成

`src/components/ProfileForm.tsx` を作成して、以下のコードを**すべて**入力してください：

```typescript
'use client';

import { useState, useRef } from 'react';
import { useRouter } from 'next/navigation';
import Image from 'next/image';
import { Camera, User, Loader2 } from 'lucide-react';
import { uploadAvatar } from '@/lib/supabase';

interface ProfileFormProps {
  user: {
    id: string;
    name: string | null;
    email: string;
    image: string | null;
    bio: string | null;
    instrument: string | null;
  };
}

export function ProfileForm({ user }: ProfileFormProps) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState('');

  // フォームの状態
  const [name, setName] = useState(user.name || '');
  const [bio, setBio] = useState(user.bio || '');
  const [instrument, setInstrument] = useState(user.instrument || '');
  const [imageUrl, setImageUrl] = useState(user.image || '');
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  // 画像選択
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // ファイルサイズチェック（5MB以下）
    if (file.size > 5 * 1024 * 1024) {
      alert('画像サイズは5MB以下にしてください');
      return;
    }

    // ファイルタイプチェック
    if (!file.type.startsWith('image/')) {
      alert('画像ファイルを選択してください');
      return;
    }

    setImageFile(file);

    // プレビュー用のURLを作成
    const url = URL.createObjectURL(file);
    setPreviewUrl(url);
  };

  // フォーム送信
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setLoading(true);

    try {
      let finalImageUrl = imageUrl;

      // 新しい画像がある場合、アップロード
      if (imageFile) {
        setUploading(true);
        finalImageUrl = await uploadAvatar(imageFile, user.id);
        setUploading(false);
      }

      // プロフィールを更新
      const response = await fetch('/api/profile', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: name.trim(),
          bio: bio.trim(),
          instrument: instrument.trim(),
          image: finalImageUrl,
        }),
      });

      const data = await response.json();

      if (!response.ok) {
        setError(data.error || 'プロフィールの更新に失敗しました');
        setLoading(false);
        return;
      }

      alert('プロフィールを更新しました');
      router.push(`/users/${user.id}`);
      router.refresh();
    } catch (err) {
      setError('エラーが発生しました');
      setLoading(false);
    }
  };

  // 現在表示する画像URL
  const displayImageUrl = previewUrl || imageUrl;

  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6">
      {/* エラーメッセージ */}
      {error && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">
          {error}
        </div>
      )}

      {/* アバター画像 */}
      <div className="mb-8">
        <label className="block text-sm font-medium text-gray-700 mb-4">
          プロフィール画像
        </label>

        <div className="flex items-center gap-6">
          {/* 画像プレビュー */}
          <div className="relative">
            {displayImageUrl ? (
              <Image
                src={displayImageUrl}
                alt="Profile"
                width={120}
                height={120}
                className="rounded-full object-cover"
              />
            ) : (
              <div className="w-30 h-30 rounded-full bg-gray-200 flex items-center justify-center">
                <User size={60} className="text-gray-400" />
              </div>
            )}

            {/* アップロード中のオーバーレイ */}
            {uploading && (
              <div className="absolute inset-0 bg-black bg-opacity-50 rounded-full flex items-center justify-center">
                <Loader2 className="animate-spin text-white" size={32} />
              </div>
            )}
          </div>

          {/* 画像選択ボタン */}
          <div>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
              onChange={handleImageSelect}
              className="hidden"
            />
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              disabled={uploading || loading}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Camera size={20} />
              画像を選択
            </button>
            <p className="text-xs text-gray-500 mt-2">
              JPG, PNG, GIF（最大5MB）
            </p>
          </div>
        </div>
      </div>

      {/* 名前 */}
      <div className="mb-6">
        <label
          htmlFor="name"
          className="block text-sm font-medium text-gray-700 mb-2"
        >
          名前 <span className="text-red-500">*</span>
        </label>
        <input
          id="name"
          type="text"
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="山田 太郎"
          required
        />
      </div>

      {/* 自己紹介 */}
      <div className="mb-6">
        <label
          htmlFor="bio"
          className="block text-sm font-medium text-gray-700 mb-2"
        >
          自己紹介
        </label>
        <textarea
          id="bio"
          value={bio}
          onChange={(e) => setBio(e.target.value)}
          rows={4}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="自己紹介を入力してください..."
        />
        <p className="text-xs text-gray-500 mt-1">
          {bio.length}/500文字
        </p>
      </div>

      {/* 担当楽器 */}
      <div className="mb-8">
        <label
          htmlFor="instrument"
          className="block text-sm font-medium text-gray-700 mb-2"
        >
          担当楽器
        </label>
        <input
          id="instrument"
          type="text"
          value={instrument}
          onChange={(e) => setInstrument(e.target.value)}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="例: ギター、ボーカル、ドラム"
        />
      </div>

      {/* ボタン */}
      <div className="flex gap-4">
        <button
          type="submit"
          disabled={loading || uploading}
          className="flex-1 px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
              <Loader2 className="animate-spin" size={20} />
              更新中...
            </>
          ) : (
            '保存'
          )}
        </button>

        <button
          type="button"
          onClick={() => router.back()}
          disabled={loading || uploading}
          className="px-6 py-3 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          キャンセル
        </button>
      </div>
    </form>
  );
}
```

### Step 5: Supabaseのセットアップ

**5-1.** Supabase Dashboard で「Storage」→「Create bucket」

**5-2.** バケット名: `avatars`、Public: ✓（有効）

**5-3.** `.env.local` に環境変数を追加（既に設定済みの場合はスキップ）

```env
NEXT_PUBLIC_SUPABASE_URL=your-supabase-url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-supabase-anon-key
```

**5-4.** Supabaseクライアントをインストール

```bash
npm install @supabase/supabase-js
```

### Step 6: 動作確認

**6-1.** http://localhost:3000/profile を開く

**6-2.** 画像を選択してプレビューを確認

**6-3.** 名前、自己紹介、担当楽器を入力

**6-4.** 「保存」ボタンをクリック

→ ✅ プロフィールが更新され、ユーザー詳細ページにリダイレクトされます！

---

## 17.4 確認チェックリスト

Chapter 17の内容を理解できたか確認しましょう。

### ユーザー一覧

- [ ] ユーザー一覧ページを実装できる
- [ ] 役割による色分けを実装できる
- [ ] アバター画像を表示できる
- [ ] 統計情報を表示できる
- [ ] カードレイアウトを実装できる

### 管理者機能

- [ ] 役割変更APIを実装できる
- [ ] 管理者権限のチェックができる
- [ ] 自分自身の役割変更を防止できる
- [ ] 役割変更ボタンを実装できる

### プロフィール編集

- [ ] プロフィール更新APIを実装できる
- [ ] Supabase Storageのセットアップができる
- [ ] 画像アップロード機能を実装できる
- [ ] プレビュー表示ができる
- [ ] フォームのバリデーションができる

### ユーザー詳細

- [ ] ユーザー詳細ページを実装できる
- [ ] プロフィール情報を表示できる
- [ ] 活動履歴を表示できる
- [ ] 統計情報を表示できる

---

## まとめ

この章では、ユーザー管理とプロフィール編集機能を実装しました。

### 🎓 この章で学んだこと

#### ユーザー管理
- ✅ ユーザー一覧の表示
- ✅ 役割による色分け
- ✅ 統計情報の集計
- ✅ アバター画像の表示

#### 管理者機能
- ✅ 役割変更API
- ✅ 権限チェック
- ✅ 楽観的UIによる更新

#### プロフィール編集
- ✅ Supabase Storageのセットアップ
- ✅ 画像アップロード機能
- ✅ プレビュー表示
- ✅ プロフィール情報の更新

#### ユーザー詳細
- ✅ プロフィール情報の表示
- ✅ 活動履歴の表示
- ✅ 統計情報の可視化

### 💡 重要なポイント

#### Supabase Storage の画像アップロード

```typescript
// 画像をアップロード
const { data, error } = await supabase.storage
  .from('avatars')
  .upload(filePath, file, {
    cacheControl: '3600',
    upsert: false,
  });

// 公開URLを取得
const { data: publicUrlData } = supabase.storage
  .from('avatars')
  .getPublicUrl(filePath);
```

#### 役割変更の権限チェック

```typescript
// 管理者のみ
if (session.user.role !== 'admin') {
  return { error: '管理者権限が必要です' };
}

// 自分自身は変更不可
if (userId === session.user.id) {
  return { error: '自分自身の役割は変更できません' };
}
```

### 🚀 次の章への準備

次の章（Chapter 18: 画像アップロード機能）では、以下を学びます：

1. **投稿画像のアップロード**
   - 複数画像のアップロード
   - 画像プレビュー

2. **画像の最適化**
   - next/imageの活用
   - リサイズ処理

**準備すること:**
- ✅ Supabase Storageが正しく動作することを確認
- ✅ アバター画像のアップロードが成功することを確認

---

[← 前の章：第16章 活動スケジュール機能の実装](16-活動スケジュール機能の実装.md) | [目次に戻る](00-目次.md) | [次の章へ：第18章 画像アップロード機能 →](18-画像アップロード機能.md)
