...
> **この章では、実際にプロジェクトの関数とコンポーネントの単体テストを書きます**

## 🛠️ **ハンズオン作業**

**⚠️ 重要**: この章では実際に手を動かしてテストコードを書きます。各セクションの指示に従って、テストファイルを作成して、以下のコードを**すべて**入力してください：

> 💡 **前提**: 第26章「テスト環境の基礎」でJestのセットアップが完了していることを確認してください。
...
### Step 2: `src/lib/__tests__` フォルダを作成
...
### Step 3: `src/lib/__tests__/utils.test.ts` を作成して、以下のコードを**すべて**入力してください：

```typescript
import { formatDate, truncateText, getYouTubeId, validateEmail } from '../utils';

describe('formatDate', () => {
  it('Dateオブジェクトを YYYY-MM-DD 形式にフォーマット', () => {
...
describe('getYouTubeId', () => {
  ...
  it('複数URL配列に不正なURLが混ざっていても正しいIDだけ抽出', () => {
    const urls = [
...
```
 PASS  src/lib/__tests__/utils.test.ts
  formatDate
    ✓ Dateオブジェクトを YYYY-MM-DD 形式にフォーマット (3 ms)
    ✓ 文字列の日付をフォーマット (1 ms)
    ✓ nullの場合は空文字を返す
    ✓ 不正な日付は空文字を返す (1 ms)
  truncateText
...
## 27.3 Reactコンポーネントのテスト

### Step 5: `src/components/__tests__` フォルダを作成

```bash
mkdir -p src/components/__tests__
```

### Step 6: `src/components/__tests__/LikeButton.test.tsx` を作成して、以下のコードを**すべて**入力してください：
...
describe('LikeButton', () => {
  ...
  it('既にいいね済みの場合、ハートが塗りつぶされる', () => {
    render(<LikeButton postId="1" initialLiked={true} initialCount={5} />);
    
    const button = screen.getByRole('button');
    expect(button).toHaveTextContent('5');
    // SVGのfillプロパティで確認（実装に応じて調整）
  });

  it('ボタンをクリックするといいねが追加される', async () => {
  ...
  it('いいね済みのボタンをクリックするといいねが解除される', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
...
## 27.5 その他のコンポーネントのテスト

### Step 9: YouTubeEmbedコンポーネントのテスト

`src/components/__tests__/YouTubeEmbed.test.tsx` を作成して、以下のコードを**すべて**入力してください：
...
1. **ユーティリティ関数**: formatDate, truncateText, getYouTubeId, validateEmail
2. **Reactコンポーネント**: LikeButton, YouTubeEmbed
3. **モックの使用**: fetch APIのモック
4. **カバレッジ測定**: テスト範囲の確認

次章では、複数のコンポーネントやAPIを組み合わせた結合テストを実装します。
...
| エラー | 原因 | 解決方法 |
|--------|------|---------|
| `Cannot find module '@/lib/utils'` | パスエイリアスの設定ミス | `jest.config.js` の `moduleNameMapper` を確認 |
| `toBeInTheDocument is not a function` | jest-dom が読み込まれていない | `jest.setup.js` で `@testing-library/jest-dom` をインポート |
| `ReferenceError: fetch is not defined` | Node.js に fetch がない | グローバルに fetch のモックを設定 |
| テストが見つからない | ファイル名が間違っている | `*.test.ts` または `*.spec.ts` の形式か確認 |
...
[← 前の章：第26章 テスト環境の基礎](26-テスト環境の基礎.md) | [目次に戻る](00-目次.md) | [次の章へ：第28章 結合テストの実装 →](28-結合テストの実装.md)
