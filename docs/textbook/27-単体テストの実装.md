...
**⚠️ 重要**: この章では実際に手を動かしてテストコードを書きます。各セクションの指示に従って、テストファイルを作成してください。
...
### Step 3: `src/lib/__tests__/utils.test.ts` を開いて、以下のコードを入力
...
### Step 6: `src/components/__tests__/LikeButton.test.tsx` を開いて、以下のコードを入力
...
`src/components/__tests__/YouTubeEmbed.test.tsx` を開いて、以下を入力：
