...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger, account }) {
      ...
      console.log('[NextAuth JWT] Token after:', JSON.stringify(token, null, 2))
      return token
    },
...
Prisma Clientのインスタンスを管理するファイルです。
...
【現在の実装】
  - admin: 管理者（全ての操作が可能）
  - member: 一般メンバー（閲覧、いいね、コメント）

【拡張例】
  - superadmin: 最高管理者（ユーザー管理も可能）
  - moderator: モデレーター（投稿の編集・削除のみ）
  - guest: ゲスト（閲覧のみ）
...
[← 前の章：第27章 ページコンポーネントの詳細解説](27-ページコンポーネントの詳細解説.md) | [目次に戻る](00-目次.md) | [次の章へ：付録A 用語集 →](付録A-用語集.md)
