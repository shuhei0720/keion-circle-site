# 第 15 章：イベント管理機能の実装【ハンズオン】

> **この章では、ライブやリハーサルなどのイベント管理機能を実装します**

## 🛠️ **ハンズオン作業**

**⚠️ 重要**: この章では実際に手を動かしてイベント管理機能を実装します。各セクションの指示に従って、イベントの作成・管理・参加登録機能を作っていきましょう。
...
`prisma/schema.prisma` を開いて、Event モデルが**以下の通り定義されていること**を確認してください。
...
`src/app/events/page.tsx` を作成して、以下のコードを**すべて**入力してください：
...
http://localhost:3000/events を開いてください。
...
`src/app/api/events/route.ts` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/events/new/page.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/components/EventForm.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/events/[id]/page.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/components/ParticipationButton.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/api/events/[id]/participate/route.ts` を作成して、以下のコードを**すべて**入力してください：
...
`src/components/DeleteEventButton.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/api/events/[id]/route.ts` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/events/[id]/edit/page.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/components/CreatePostFromEventButton.tsx` を作成して、以下のコードを**すべて**入力してください：
...
`src/app/posts/new/page.tsx` を開いて、以下のコードで**完全に上書き**してください。

> 💡 **説明**: イベントから投稿作成する際に、イベント情報（eventId、タイトル、内容）をクエリパラメータで受け取れるようにします。
...
> 💡 **説明**: イベントから渡されるテンプレート情報（タイトル、内容）を初期値として設定できるようにします。
