...
**初心者への補足：**
> 💡 データベースは「エクセルの超強化版」です。たくさんの表を作って、それらを関連付けて管理できます。SQLiteは「個人用ノート」、PostgreSQLは「図書館」みたいなイメージです。
...
詳細は[第36章：デプロイと運用](./36-デプロイと運用.md)で解説します。
...
graph TB
    ...
    A -->|1-3章| A1[Web開発の基礎]
    B -->|4-7章| B1[React, Next.js, TS, Tailwind]
    C -->|8-9章| C1[開発環境]
    D -->|10-13章| D1[認証とDB]
    E -->|14-16章| E1[API Routes]
    F -->|17-24章| F1[ページとコンポーネント]
    G -->|25-27章| G1[デザイン]
    H -->|28-35章| H1[最適化・テスト]
    I -->|36章| I1[本番環境]
...
**第Ⅵ部：フロントエンド実装（第17-24章）**
- ページコンポーネント
- 投稿機能
- コメント機能
- スケジュール機能

**第Ⅶ部：スタイリングとUI/UX（第25-27章）**
- Tailwind CSSの活用
- レスポンシブデザイン
- アクセシビリティ
...
- バックエンド: API Routes, Prisma, PostgreSQL/SQLite
- 認証: NextAuth.js v5
